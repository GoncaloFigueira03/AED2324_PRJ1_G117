...
vector<studentClassChange> Reader::read_changes() {
    vector<studentClassChange> changes;

    ifstream fileChanges;
    fileChanges.open("../changes.csv");
    if (!fileChanges.is_open()) {
        return changes;
    }

    string buffer;
    getline(fileChanges, buffer, '\n');
    while (getline(fileChanges, buffer, '\n')) {
        stringstream line(buffer);
        string newBuffer;
        studentClassChange newChange;
        getline(line, newBuffer, ',');
        newChange.type = newBuffer;
        getline(line, newBuffer, ',');
        newChange.studentCode = newBuffer;
        getline(line, newBuffer, ',');
        newChange.studentName = newBuffer;
        getline(line, newBuffer, ',');
        newChange.oldUcCode = newBuffer;
        getline(line, newBuffer, ',');
        newChange.oldClassCode = newBuffer;
        getline(line, newBuffer, ',');
        newChange.newUcCode = newBuffer;
        getline(line, newBuffer, '\r');
        newChange.newClassCode = newBuffer;
