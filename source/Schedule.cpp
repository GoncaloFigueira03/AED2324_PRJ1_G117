...
void Scheduler::printRequestSuccessLogs() {
    vector<studentClassChange> readChanges = studentClassesReader.read_changes();
    cout << "The Following Requests were Successful:" << endl;
    for (auto it_readChanges:readChanges) {
        if (it_readChanges.type == "Add") {
            cout << GREEN << "UP" << it_readChanges.studentCode << ", " << it_readChanges.studentName << ", Added: [" << it_readChanges.newUcCode << ", "
                  << it_readChanges.newClassCode << "]" << RESET << endl;
        }
        else if (it_readChanges.type == "Remove") {
            cout << RED << "UP" << it_readChanges.studentCode << ", " << it_readChanges.studentName << ", Removed: [" << it_readChanges.oldUcCode << ", "
                  << it_readChanges.oldClassCode << "]" << RESET << endl;
        }
        else if (it_readChanges.type == "Change") {
            cout << YELLOW << "UP" << it_readChanges.studentCode << ", " << it_readChanges.studentName << ", Changed: [" << it_readChanges.oldUcCode << ", "
                  << it_readChanges.oldClassCode << "] -> [" << it_readChanges.newUcCode << ", " << it_readChanges.newClassCode << "]" << RESET << endl;
        }
    }
}
